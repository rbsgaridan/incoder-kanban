...
const cards = ref<KanbanCard[]>([
  ...
  },
  {
    ...
    priority: Priority.HIGH,
    tags: [
      { id: 4, label: 'DevOps', color: '#f59e0b' }
    ]
  },
  {
    id: 4,
    ...
    tags: [
      { id: 5, label: 'Documentation', color: '#06b6d4' }
    ]
  },
  {
    id: 5,
...
const handleCardMoved = (event: CardMoveEvent) => {
  ...
  eventLog.value.unshift(
    `Moved "${event.card.title}" from ${getColumnTitle(event.fromColumnId)} to ${getColumnTitle(event.toColumnId)}`
  );
