...
const swimlanesEnabled = computed(() => {
  return (
    boardOptions.value.enableSwimlanes &&
...
const emitBoardChange = (updatedCards?: KanbanCard[]) => {
  const boardChange: BoardChangeEvent = {
    ...
    swimlanes: swimlanesEnabled.value
      ? state.currentSwimlanes.value
      : undefined,
  };
...
<template>
  ...
  >
    <!-- Loading overlay -->
    <div v-if="loading" class="kanban-board-loading-overlay">
      <i class="pi pi-spin pi-spinner" style="font-size: 2rem"></i>
    </div>

    <!-- Board toolbar slot -->
    <div v-if="$slots.toolbar" class="kanban-board-toolbar">
      <slot
        name="toolbar"
        :columns="state.currentColumns.value"
    ...
    <div class="kanban-board-container">
      <!-- Without swimlanes -->
      <div v-if="!swimlanesEnabled" class="kanban-board-columns">
        <KanbanColumn
      ...
      <!-- With swimlanes -->
      <div v-else class="kanban-board-swimlanes">
        <div
        ...
        >
          ...
          <div class="kanban-board-swimlane-columns">
            <KanbanColumn
              ...
              @card-dragend="handleCardDragEnd"
              @column-dragover="
                handleColumnDragOver(column.id, swimlane.id, $event)
              "
              @column-dragleave="handleColumnDragLeave"
